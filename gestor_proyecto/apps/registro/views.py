...
def addregistro(request):
    if request.method == "POST":
        ...
        geopos = LocalizacionForm(request.POST, prefix='geopos')
        cale = calendarioForm(request.POST, prefix='cale')
        ...
        if infobasica.is_valid() and geopos.is_valid() and cale.is_valid() and formset.is_valid():
            ...
            return redirect('/')
        ctx = {'infobasica': infobasica, 'gp': geopos, 'cale': cale, 'formset': formset }
    else:
        infobasica = InfoBasicaForm()
        geopos = LocalizacionForm(prefix='geopos')
        calendario = calendarioForm(prefix='cale')
        formset = FechasFormSet(prefix='fechas')
